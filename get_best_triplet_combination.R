...
for( i in 1:(length(layers)-2) ){
	for( j in (i+1):(length(layers)-1) ){
		for( k in (j+1):(length(layers)) ){
			i.low.R <- c(i.low.R, i)
			j.low.R <- c(j.low.R, j)
			k.low.R <- c(k.low.R, k)
		}
...
sum.xxyy.diff.all.1 <- c()
sum.xxyy.diff.all.2 <- c()
sum.xxyy.diff.all.3 <- c()
sum.xxyy.diff.all   <- c()
for( s in 1:combinations ){
	sum.xxyy.diff.all.1 <- c(sum.xxyy.diff.all.1, 0)
	sum.xxyy.diff.all.2 <- c(sum.xxyy.diff.all.2, 0)
	sum.xxyy.diff.all.3 <- c(sum.xxyy.diff.all.3, 0)
	sum.xxyy.diff.all   <- c(sum.xxyy.diff.all  , 0)
}
...
for( r in 1:nrow(input_train) ){
	...
	y.c[[r]] <- unlist( y[r,], use.names=FALSE)
        ...
        d2_i.j    <- ( (x.c[[r]][j.low.R] - x.c[[r]][i.low.R])^2 +
                       (y.c[[r]][j.low.R] - y.c[[r]][i.low.R])^2 )

        d2_i.k    <- ( (x.c[[r]][k.low.R] - x.c[[r]][i.low.R])^2 +
                       (y.c[[r]][k.low.R] - y.c[[r]][i.low.R])^2 )

        delta.x_i.j <- (x.c[[r]][j.low.R] - x.c[[r]][i.low.R])
        delta.x_i.k <- (x.c[[r]][k.low.R] - x.c[[r]][i.low.R])

        delta.y_i.j <- (y.c[[r]][j.low.R] - y.c[[r]][i.low.R])
        delta.y_i.k <- (y.c[[r]][k.low.R] - y.c[[r]][i.low.R])
        ...
        r.c <- sqrt( (x.c[[r]][i.low.R] - x0.c)^2 +
		     (y.c[[r]][i.low.R] - y0.c)^2 )

	for( c in 1:length(r.c) ){
		fit.xxyy   <- sqrt( (x.c[[r]] - x0.c[c])^2 + (y.c[[r]] - y0.c[c])^2 )
		fit.cosine <- (x.c[[r]] - x0.c[c]) / fit.xxyy
		fit.sine   <- (y.c[[r]] - y0.c[c]) / fit.xxyy
		xx <- x0.c[c] + r.c[c]*fit.cosine
		yy <- y0.c[c] + r.c[c]*fit.sine
		
		xxyy.diff <- sqrt( (x.c[[r]] - xx)^2 + (y.c[[r]] - yy)^2 )
		if( pt.train[r]/0.0006 <= 5000 ){
			sum.xxyy.diff.all.1[c] <- sum.xxyy.diff.all.1[c] +
		    		                  sum(xxyy.diff)
		}
		else if( pt.train[r]/0.0006 > 5000 & pt.train[r]/0.0006 < 20000 ){
			sum.xxyy.diff.all.2[c] <- sum.xxyy.diff.all.2[c] +
                                                  sum(xxyy.diff)
		}
		else{
			sum.xxyy.diff.all.3[c] <- sum.xxyy.diff.all.3[c] +
                                                  sum(xxyy.diff)
		}
		sum.xxyy.diff.all[c] <- sum.xxyy.diff.all[c] +
			                sum(xxyy.diff)
	}
}
cat("\n")
...
png("momentum/fit_2020_06_23_v4/sum.xy.diff_vs_combinations.png",
#png("momentum/fit_2020_06_23_v4/sum.xy.diff_vs_combinations_normalized.png",
#png("momentum/fit_2020_06_23_v4/sum.xy.diff_vs_combinations_R.lt.5m.png",
#png("momentum/fit_2020_06_23_v4/sum.xy.diff_vs_combinations_R.gt.5m.lt.20m.png",
#png("momentum/fit_2020_06_23_v4/sum.xy.diff_vs_combinations_R.gt.20m.png",
...
png("momentum/fit_2020_06_23_v4/pt_vs_R_high.R.png",
    units="px", width=1600, height=1600, res=250)
plot(r.fit1000,
     pt.train,
...
png("momentum/fit_2020_06_23_v4/hist_R.png",
    units="px", width=1600, height=1600, res=250)
#hist(r.fit,
#     breaks=200,
...
#png("momentum/fit_2020_06_23_v4/hist_pt_1-4.png",
#    units="px", width=1600, height=1600, res=250)
...
png("momentum/fit_2020_06_23_v4/hist_x_difference.png",
    units="px", width=1600, height=1600, res=250)
for( l in layers ){
...
legend( "topright", legend=sprintf( "Layer %2d, (sd, BW) = (%3.0f, %1.3f) mm",
...
png("momentum/fit_2020_06_23_v4/hist_y_difference.png",
    units="px", width=1600, height=1600, res=250)
for( l in layers ){
        ...
        if( l == 1 ){
                plot( y.diff.hist[[l]]$mids, y.diff.hist[[l]]$counts,
                      ...
                      ylab="Frequency",
                      main="21k Tracks",
                      ylim=c(0,6000), lwd=2, col=rgb.color[[l]], type="l")
        }
        else{
...
png("momentum/fit_2020_06_23_v4/hist_xy_difference.png",
    units="px", width=1600, height=1600, res=250)
for( l in layers ){
...
png("momentum/fit_2020_06_23_v4/count_fits_outside_diff.png",
    units="px", width=1600, height=1600, res=250)
...
# 836  1641  7662  9062 11350 12406 15184 15402 16446 16561 16845
# pt
# 101.816520   5.849100  50.638954  27.343701  50.638954  43.062082
# 50.638954  68.913653  50.638954  29.971027   3.484532
####

#high.diff <- c( 1548,  1641,  7689,  8697,  9954, 12088,
#	       13339, 15209, 16472, 20367, 20632)
high.diff <- c(836,  1641,  7662,  9062, 11350, 12406,
	       15184, 15402, 16446, 16561, 16845)
png("momentum/fit_2020_06_23_v4/fit.circle_high.diff.2.png",
    units="px", width=1600, height=1600, res=250)
...
png("momentum/fit_2020_06_23_v4/fit.circle_high.pt.png",
    units="px", width=1600, height=1600, res=250)
plot(unlist(x.c, use.names=F), unlist(y.c, use.names=F),
     xlab="x (mm)", ylab="y (mm)",
...
#png("momentum/fit_2020_06_23_v4/sum.all.diff.png",
png("momentum/fit_2020_06_23_v4/sum.all.diff_0-150_mm.png",
    units="px", width=1600, height=1600, res=250)
#hist( sum.diff.all, breaks=100,
hist( sum.diff.all[sum.diff.all < 150], breaks=100,
#      xlim=c(0,300),
#      ylim=c(0,4000),
...
png("momentum/fit_2020_06_23_v4/pt_vs_R_all.pT_and_lower.pT.sum.diff.cut20.png",
    units="px", width=1600, height=1600, res=250)
...
#####################################################
#png("momentum/fit_2020_06_23_v4/pt_vs_R_lower.pT.png",
#    units="px", width=1600, height=1600, res=250)
...
png("momentum/fit_2020_06_23_v4/hist_R_lower.pT.sum.diff.cut80.png",
    units="px", width=1600, height=1600, res=250)
...
legend( "topright", legend=c("21k Tracks", "19k Tracks"),
        fill=c(alpha(rgb.color[[1]], 1), alpha(rgb.color[[3]], 0.7)) )
dev.off()

#####################################################
png("momentum/fit_2020_06_23_v4/hist_all.pt_and_low.pt.sum.diff.cut80.png",
    units="px", width=1600, height=1600, res=250)
