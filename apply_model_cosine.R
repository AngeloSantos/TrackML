...
output_folder               = "figures/application_2020_09_19_v2"

update_dataset = FALSE

volume.id <- c(13, 13, 13, 13, 17, 17)
layer.id  <- c( 2,  4,  6,  8,  2,  4)
...
for( l in layers ){
	...
	for( p in 1:nrows_sequence ){
		...
		# Build predicted hit with (x_5, y_5, z_5) coordinates.
		# Then move the origing of such hit to the hit in the
		# fourth layer (x_4, y_4, z_4)
                pred.displaced <- c( x_5 = c(pred.x - application_4hits[p, "x_4"]),
		    		     y_5 = c(pred.y - application_4hits[p, "y_4"]),
		   		     z_5 = c(pred.z - application_4hits[p, "z_4"]) )

		# Take only hits from the layer in analysis
		# based on volume and layer IDs
		layer.index <- which(application_source[,"volume_id_5"]==volume.id[l] &
				     application_source[,"layer_id_5"]==layer.id[l])
		layer_source <- application_source[layer.index, ]

		# Build matrix with 3D hits from the source dataset.
		#hits.source <- rbind( layer_source[, "x_5"],
		#		      layer_source[, "y_5"],
		#		      layer_source[, "z_5"] )
		...
		# Compute Euclidean distance between predicted hit
		# and all hits from the source dataset
                leastDistance <- sqrt( (pred.x - layer_source[, "x_5"])^2 +
                                       (pred.y - layer_source[, "y_5"])^2 +
                                       (pred.z - layer_source[, "z_5"])^2 )

		# Check how many hits are inside a give radius
		# with origin in the predicted hit.
		#
		# If there are non zero hits around the predicted one,
		# move the origin of such restricted hits to the hit in the
		# fourth layer.
		#
		# Otherwise, move the origin of all hits in the layer in analysis.
		hits.restricted <- which( leastDistance < lower.radius )
		if( length(hits.restricted) < 1 )
			hits.restricted <- which( leastDistance < medium.radius )
		if( length(hits.restricted) < 1 )
			hits.restricted = nrow(layer_source)

		hits.source <- rbind( x_5=c(layer_source[hits.restricted, "x_5"]
					    - application_4hits[p, "x_4"]), # x
			              y_5=c(layer_source[hits.restricted, "y_5"]
					    - application_4hits[p, "y_4"]), # y
                                      z_5=c(layer_source[hits.restricted, "z_5"]
					    - application_4hits[p, "z_4"]) )# z

		# Compute cosine similarity between predicted hit
		# and all hits from source dataset
		cos.similarity <- cosine(pred.displaced, hits.source)

		# The best hit from source is that with the maximum
		# cosine similarity
                max.cosine = max(cos.similarity)

		# Get index of hit with the best cosine similarity
                near.index = layer.index[hits.restricted[which( cos.similarity
							        == max.cosine )[1]]]

		# Compute Euclidean distantce between predicted hit and
		# nearest hit based on cosine similarity
		dist.pred_near = leastDistance[near.index]

		# Get sample ID of hit with the best cosine similarity
...
########################################
# Set column names for the dataset of maximum cosine similarity
# between predicted and nearest hits
message("*******************************************")
